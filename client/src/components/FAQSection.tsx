...
import { ChevronDown, ChevronUp, Phone, MessageCircleQuestion, HelpCircle, Mail, Share2 } from 'lucide-react';

const FAQSection = () => {
  // Categories for FAQs with icons
  const faqCategories = [
    { id: 'all', name: 'All Questions', icon: <MessageCircleQuestion className="h-5 w-5" /> },
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer: "After your order ships, you'll receive a tracking number via email and SMS. You can track your package in real-time through our website's 'Order Tracking' section or directly through our shipping partner's website using your provided tracking number.",
      category: 'ordering'
    },
    {
      question: "What is your return and exchange policy?",
      answer: "We offer a 7-day return policy for unopened items in their original packaging. For damaged or defective products, please contact our support team within 48 hours of delivery with photos of the damaged items. Exchanges are processed after we receive the returned items.",
      category: 'support'
    }
  ];
  ...
  return (
    <section id="faq" className="py-20 relative overflow-hidden">
            ...
            {filteredFaqs.length > 0 ? (
              filteredFaqs.map((faq, index) => (
                <motion.div
                  ...
                  className={`border border-[#333] rounded-xl overflow-hidden bg-gradient-to-b from-[#1A1A1A] to-[#0D0D0D] shadow-lg transition-all duration-300 ${
                    activeIndex === index ? 'border-accent/30' : 'hover:border-accent/20'
                  }`}
                >
                  <button
                  ...
                  >
                    <h3 className={`font-semibold text-lg transition-colors duration-300 ${
                      activeIndex === index ? 'text-accent' : 'group-hover:text-accent/80'
                    }`}>{faq.question}</h3>
                    <div className={`${
                      activeIndex === index ? 'bg-accent text-white' : 'bg-[#222] text-accent group-hover:bg-accent/20'
                    } p-2 rounded-full transition-colors duration-300`}>
                      {activeIndex === index ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                    </div>
                  </button>
                  <div 
              ...
              >
                ...
                <p className="text-gray-400">
                  There are no questions in this category yet. Try selecting a different category.
                </p>
              </motion.div>
            )}
          </div>
      ...
      {/* Animated elements */}
      <div className="absolute top-1/4 right-8 w-2 h-2 bg-accent rounded-full animate-ping" style={{ animationDuration: '2s' }}></div>
      <div className="absolute bottom-1/4 left-8 w-2 h-2 bg-accent rounded-full animate-ping" style={{ animationDuration: '3s', animationDelay: '0.5s' }}></div>
      <div className="absolute top-2/4 left-2/3 w-2 h-2 bg-white rounded-full animate-ping" style={{ animationDuration: '4s', animationDelay: '1s' }}></div>
