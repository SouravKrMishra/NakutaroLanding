    ...
    try {
      ...
      const paymentResponse = await fetch("/api/payments/phonepe/initiate", {
        ...
        body: JSON.stringify({
          ...
          redirectUrl: `${window.location.origin}/order-success?orderId=${orderResult.order.id}`,
            ...
            <Card className="bg-[#1a1a1a] border-[#333] sticky top-32">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  {/* Test Buttons for Demo - Only show in development */}
                  {process.env.NODE_ENV === "development" && (
                    <div className="mt-4 p-3 bg-yellow-900/20 border border-yellow-500/30 rounded-lg">
                      <p className="text-yellow-400 text-sm font-medium mb-3 text-center">
                        🧪 Test Payment Flows (Development Only)
                      </p>
                        ...
                        <Button
                          onClick={async () => {
                            ...
                            try {
                              ...
                              const merchantTransactionId = `TXN_${Date.now()}_TEST_SUCCESS`;
                              ...
                              const orderData = {
                                items: orderItems,
                                shippingInfo,
                                paymentMethod: "PHONEPE", // Use valid payment method
                                subtotal: total,
                                shippingCost,
                                total: finalTotal,
                                merchantTransactionId,
                              ...
                              // Store test transaction details
                              localStorage.setItem(
                                "phonepe_transaction",
                                JSON.stringify({
                              ...
                              toast({
                                ...
                                description: `Order #${orderResult.order.orderNumber} has been created and your cart has been cleared.`,
                                variant: "default",
                              });

                              // Redirect to order success page
                              setLocation(
                                `/order-success?orderId=${orderResult.order.id}&test=true`
                              );
                            } catch (error: any) {
                          ...
                          disabled={isProcessing}
                          className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 text-sm disabled:opacity-50"
                        >
                          {isProcessing ? (
                            <div className="flex items-center">
                        ...
                        <Button
                          onClick={() => {
                            ...
                            toast({
                              ...
                              description:
                                "This is a test failure simulation. Your payment could not be processed. Please try again or use a different payment method.",
                              variant: "destructive",
                            });
                          }}
                      ...
                      <p className="text-yellow-300 text-xs mt-2 text-center">
                        These buttons simulate payment outcomes for testing
                      </p>
                    </div>
                  )}
